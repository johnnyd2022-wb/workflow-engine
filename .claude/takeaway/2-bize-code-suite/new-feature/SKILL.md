...
Scaffold the blueprint (adjust to the repo's existing pattern if one exists; consistency beats this template):

```
app/features/<slug>/
  __init__.py      # bp = Blueprint("<slug>", __name__, url_prefix="/<slug>")
  routes.py        # thin handlers: parse, authorize, call service, respond
  service.py       # business logic, no Flask imports
  schemas.py       # explicit request/response schemas (mass-assignment guard)
  models.py        # only if spec's data model says changes
tests/unit/<slug>/test_service.py
tests/integration/<slug>/test_routes.py
```

Register the blueprint in the app factory. Then build to the spec:

- Every route authorized; every tenant-scoped query org-filtered from the first line, not patched in later. The security audit will check this; write it so the audit is boring.
- Unit tests named to ACs (`test_ac1_...`), one per criterion minimum, plus the unhappy paths. Run until green: `pytest tests/unit/<slug> tests/integration/<slug> -q`.
...
Report to the user only when true: spec approved, unit tests green with AC coverage, migrations up/down/up verified (or none), security verdict clean/patched, E2E green and flake-checked, observability instrumented, all ci gates passing, branch pushed with CI status. Set the spec's `status: built`. Present the summary as a short table of stage -> verdict -> report path, then the MR link (`glab mr create`) or push instructions.
