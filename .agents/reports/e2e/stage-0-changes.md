...
### Flake hunted and fixed before the PR

One intermittent failure (~40%) surfaced in `test_manual_inventory_add_persists_and_shows`
during pre-push flake checking. Root cause: submitting the manual-add form redirects to
`/core` while the add page's background `/system-findings` poll is still in flight; the
browser cancels it (`net::ERR_ABORTED`), which the app's `core-api.js` then logs as a
console error. Both are benign navigation-cancellation, not broken resources.

Two-part fix, both in the harness commit: the probe now ignores `ERR_ABORTED` request
failures (genuine transport failures — connection refused, DNS, timeout — still count),
and the inventory test asserts page cleanliness on the add page *before* submit rather than
on the racy post-redirect `/core` (whose cleanliness is covered on a settled load by
`test_pages_render`). Not suppressed in the app: a navigation-cancelled fetch surfaces as
`TypeError: Failed to fetch`, indistinguishable from a real network error at the JS level,
so `core-api.js` is correct to log it. Verified 5×/5× stable after the fix; full suite 3×
clean.
