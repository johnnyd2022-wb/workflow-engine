...
def attempt_automatic_sales_mapping(sales_id, products_data=None):
    ...
    try:
        ...
        for product_name, product_data in products_data['products'].items():
            ...
            """, (
                execution_id, sales_id, product_name, quantity, batch_reference,
                'auto', 0.8, f'Auto-mapped based on batch reference: {batch_reference}',
                str(uuid.uuid4())
            ))
...
def create_manual_sales_mapping(execution_id, sales_id, product_name, quantity_sold, 
                               batch_reference='', mapping_notes=''):
    """
    ...
    try:
        ...
        """, (
            execution_id, sales_id, product_name, quantity_sold, batch_reference,
            'manual', 1.0, mapping_notes, str(uuid.uuid4())
        ))
