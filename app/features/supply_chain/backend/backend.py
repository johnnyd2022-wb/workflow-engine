...
def delete_process(process_id):
    ...
    try:
        ...
        cursor.execute(
            "DELETE FROM supply_chain_connections WHERE from_process_id = %s OR to_process_id = %s",
            (process_id, process_id),
        )
...
def create_flow_through_inputs_internal(output_id, connection, cursor):
    ...
    try:
        ...
        cursor.execute(
            "SELECT sub_process_name FROM supply_chain_sub_processes WHERE id = (SELECT process_id FROM supply_chain_outputs WHERE id = %s)",
            (output_id,),
        )
...
def move_process_to_unmanaged(process_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id FROM supply_chain_connections WHERE from_process_id = %s OR to_process_id = %s",
            (process_id, process_id),
        )
...
def delete_parent_process(parent_process_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, parent_process_name FROM supply_chain_parent_processes WHERE id = %s", (parent_process_id,)
        )
        parent_process = cursor.fetchone()
        if not parent_process:
        ...
        for sub_process_id in sub_process_ids:
            ...
            # Delete connections
            cursor.execute(
                "DELETE FROM supply_chain_connections WHERE from_sub_process_id = %s OR to_sub_process_id = %s",
                (sub_process_id, sub_process_id),
            )
            connections_deleted = cursor.rowcount
...
@supply_chain_bp.route(
    "/api/supply-chain/sub-executions/<int:sub_execution_id>/complete-with-outputs", methods=["POST"]
)
