...
def validate_customer_exists(connection, cursor, customer_name):
    ...
    try:
        ...
        if not result:
            return False, api_response(False, message=f"Customer '{customer_name}' not found", status_code=404)

        return True, None

    except Exception as e:
        return False, api_response(False, message=f"Error validating customer: {str(e)}", status_code=500)
