...
Cycles: The traversal uses a visited-node set and never re-enters a node, so cycles in the graph
do not cause infinite loops; each node is processed at most once. Stress-testing with cyclic
data is still recommended to validate behavior and performance.
"""
