...
def get_matching_untracked(
    ...
    for item in items:
        ...
        if process_id:
            ...
            ex = (
                session.query(Execution)  # nosemgrep: sqlalchemy-query-in-for-loop — filters by process_id per item, not batchable without restructuring
                .filter(
...
def reconcile_via_addition(
    ...
    try:
        if untracked_item_id:
            ...
            added_unit = (unit or "").strip() or "units"
            untracked_unit = (untracked.unit or "").strip() or "units"
            ...
            added_qty_in_untracked_unit = convert_to_inventory_unit_decimal(added_qty_dec, added_unit, untracked_unit)
            ...
            reconv_in_added_unit = convert_to_inventory_unit_decimal(reconciliation_amount, untracked_unit, added_unit)
...
def reconcile_via_execution(
    ...
    untracked_unit = (untracked.unit or "").strip() or "units"
    if not are_units_compatible(out_unit, untracked_unit):
        session.rollback()
        return {"error": "Unit mismatch: output unit must be compatible with untracked item unit"}

    qty_in_untracked_unit = convert_to_inventory_unit_decimal(qty_produced, out_unit, untracked_unit)
    reconciliation_amount = min(qty_in_untracked_unit, untracked_balance)
    reconv_in_output_unit = convert_to_inventory_unit_decimal(reconciliation_amount, untracked_unit, out_unit)
    ...
    try:
        ...
        session.flush()
        # Expire so next access reloads from DB (avoids race window from re-query pattern)
        session.expire(exec_step)
...
def reconcile_output_to_untracked_reduce_only(
    ...
    output_in_untracked_unit = convert_to_inventory_unit_decimal(output_quantity, out_unit, untracked_unit)
    ...
    if effective_balance <= 0:
        ...
        if consumed_in_execution and consumed_in_execution > 0:
            ...
            effective_balance = output_in_untracked_unit

    # All amounts in untracked unit for reconciliation math.
    if stored_remaining is not None and stored_remaining >= 0:
        reconciliation_amount = min(output_in_untracked_unit, effective_balance, stored_remaining)
    ...
    surplus = output_quantity - convert_to_inventory_unit_decimal(reconciliation_amount, untracked_unit, out_unit)
