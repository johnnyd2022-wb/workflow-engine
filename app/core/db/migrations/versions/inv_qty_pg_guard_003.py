"""Explicit INSERT vs quantity-changing UPDATE semantics in inventory_items quantity trigger.

Revision ID: inv_qty_pg_guard_003
Revises: merge_inv_mov_qty_001

If your graph has no merge revision yet, point ``down_revision`` at your single head (e.g.
``inv_qty_pg_guard_002``) after resolving multiple heads with a merge migration.
"""
...
revision: str = "inv_qty_pg_guard_003"
down_revision: Union[str, None] = "merge_inv_mov_qty_001"
        ...
        BEGIN
          ...
          IF TG_OP = 'UPDATE' THEN
            ...
            IF migration_ok OR guard_ok THEN
              RETURN NEW;
            END IF;
            RAISE EXCEPTION 'inventory_items: quantity-changing UPDATE blocked (set app.inventory_qty_guard via app, or app.migration_mode for migrations)';
          END IF;
