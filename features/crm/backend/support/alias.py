...
def update_customer_alias(connection, cursor, customer_name, alias_name):
    ...
    if alias_name == new_alias_name:
        return api_response(
            False,
            message="New alias name must be different from current alias name",
            status_code=400
        )
    ...
    try:
        ...
        if alias_name not in current_aliases:
            return api_response(
                False, 
                message=f"Alias '{alias_name}' not found for this customer",
                status_code=404
            )
        ...
        if new_alias_name in current_aliases:
            return api_response(
                False, 
                message=f"New alias '{new_alias_name}' already exists for this customer",
                status_code=409
            )
...
# Legacy endpoints for backward compatibility (keep existing routes working)
@crm_bp.route('/crm-add-alias', methods=['POST'])
def crm_add_alias_legacy():
    '''
    Legacy endpoint - adding alias to customer.
    Kept for backward compatibility with existing frontend.
    '''
    print("Accessed legacy /crm-add-alias route")
    return create_customer_alias_on_request()


@crm_bp.route('/crm-update-alias', methods=['POST'])
def crm_update_alias_legacy():
    '''
    Legacy endpoint - updating alias for customer.
    Kept for backward compatibility with existing frontend.
    '''
    print("Accessed legacy /crm-update-alias route")
    return update_customer_alias_on_request()


@crm_bp.route('/crm-remove-alias', methods=['POST'])
def crm_remove_alias_legacy():
    '''
    Legacy endpoint - removing alias from customer.
    Kept for backward compatibility with existing frontend.
    '''
    print("Accessed legacy /crm-remove-alias route")
    return delete_customer_alias_on_request()


def create_customer_alias_on_request():
    ...
    try:
        ...
        # Direct implementation instead of calling new API
        alias_name = alias_name.strip()
        ...
        # Get current aliases
        cursor.execute("SELECT aliases FROM crm_customers WHERE customer = %s", (customer_name,))
        result = cursor.fetchone()
        current_aliases = result[0] if result[0] else []
        ...
        # Update the aliases array
        cursor.execute("UPDATE crm_customers SET aliases = %s WHERE customer = %s", (current_aliases, customer_name))
        connection.commit()
        
        return jsonify({"success": True, "message": f"Alias '{alias_name}' added successfully"})
    except Exception as e:
        if connection:
            connection.rollback()
...
def update_customer_alias_on_request():
    """Helper function for legacy endpoints."""
    ...
    if not customer_name or not alias_name or not new_alias_name:
        return jsonify({"success": False, "message": "Customer name, alias name and new alias name are required"})
    ...
    try:
        ...
        # Check if new alias already exists
        if new_alias_name in current_aliases:
            return jsonify({"success": False, "message": f"New alias '{new_alias_name}' already exists for this customer"})
        ...
        return jsonify({"success": True, "message": f"Alias '{alias_name}' updated to '{new_alias_name}' successfully"})
    except Exception as e:
        if connection:
            connection.rollback()
        return jsonify({"success": False, "message": str(e)})
...
def delete_customer_alias_on_request():
    """Helper function for legacy endpoints."""
    ...
    try:
        ...
        return jsonify({"success": True, "message": f"Alias '{alias_name}' removed successfully"})
    except Exception as e:
        if connection:
            connection.rollback()
        return jsonify({"success": False, "message": str(e)})
