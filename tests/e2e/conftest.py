...
def pytest_collection_modifyitems(config, items) -> None:
    """Skip every E2E test with a stated reason when a dependency is absent (D4).

    This runs at COLLECTION time — before any fixture, and crucially before
    pytest-playwright's `browser` fixture tries to launch chromium. An `e2e_preconditions`
    fixture is too late: pytest-playwright would already have attempted the launch and
    failed hard (that is exactly what broke CI). Deciding here, once, guarantees a missing
    browser is a clean skip and never a failure — the same mechanism the live_server gate
    in tests/conftest.py uses.
    """
...
def e2e_preconditions() -> None:
    """Belt-and-suspenders: the collection hook above is the real gate, but a test run
    that bypasses collection filtering (e.g. `-p no:cacheprovider` quirks) still stops
    here rather than launching against a missing dependency."""
    reason = _e2e_skip_reason()
...
class PageProbe:
    ...
    # A request the browser cancels because navigation started before it finished reports
    # as net::ERR_ABORTED. That is not a broken resource — it is a background fetch (e.g.
    # /core fires /api/core/system-findings) racing a redirect, and counting it makes any
    # test that navigates after an action flaky. Genuine transport failures
    # (ERR_CONNECTION_REFUSED, ERR_NAME_NOT_RESOLVED, ERR_FAILED, timeouts) still count.
