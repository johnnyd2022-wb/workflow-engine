#!/usr/bin/env python3
"""
Centralized API Test Runner for WhistleBird CRM
        ...
        for root, dirs, files in os.walk(test_dir):
            for pattern in test_patterns:
                ...
                for match in matches:
                    ...
                    if (os.path.isfile(test_file) and 
                        test_file.endswith('.py') and 
                        test_file not in test_files):
...
def run_api_tests():
    ...
    print_header("WhistleBird API Test Suite")
    ...
    for test_file in test_files:
        try:
            ...
            test_count = sum(tests.countTestCases() for tests in [loader.loadTestsFromTestCase(tc) for tc in test_cases])
