...
9. Identify the MR head pipeline and its SHA with `glab mr view --output json`. Monitor that exact pipeline to a terminal state. Use `glab ci status --live` only when it reports the same head SHA; otherwise poll `glab api projects/:id/pipelines/<pipeline-id>` and `glab api projects/:id/pipelines/<pipeline-id>/jobs`. Never merge, approve, enable auto-merge, or run `glab mr merge`; leave that decision to a human.
10. If the MR head pipeline fails, inspect its failed jobs and logs with `glab api projects/:id/pipelines/<pipeline-id>/jobs`, `glab ci view`, and `glab ci trace <job>`. Decide whether the failure is caused by this branch before changing anything. For a related implementation, test, lint, security, or dependency failure, diagnose the root cause and fix it:
